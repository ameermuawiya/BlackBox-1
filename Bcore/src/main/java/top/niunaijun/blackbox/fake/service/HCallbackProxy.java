...
public class HCallbackProxy implements IInjectHook, Handler.Callback {
    ...
    /*
     * Safely compares integer values to avoid NullPointerException during unboxing.
     */
    private boolean isMatchWhat(int what, Integer target) {
        return target != null && what == target;
    }
        ...
        if (!mBeing.getAndSet(true)) {
            try {
                if (BuildCompat.isPie()) {
                    if (isMatchWhat(msg.what, BRActivityThreadH.get().EXECUTE_TRANSACTION())) {
                    ...
                    if (isMatchWhat(msg.what, BRActivityThreadH.get().LAUNCH_ACTIVITY())) {
                        if (handleLaunchActivity(msg.obj)) {
                            getH().sendMessageAtFrontOfQueue(Message.obtain(msg));
                            return true;
                        }
                ...
                if (isMatchWhat(msg.what, BRActivityThreadH.get().CREATE_SERVICE())) {
